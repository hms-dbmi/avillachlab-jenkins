...
GITLAB_TOKEN=""
GITLAB_USER=""
if [ "${git_credentials_id}" = "gitlab_token" ]; then
  GITLAB_USER=$(aws secretsmanager get-secret-value --secret-id gitlab-token --query SecretString --output text|jq -r .username)
  GITLAB_TOKEN=$(aws secretsmanager get-secret-value --secret-id gitlab-token --query SecretString --output text|jq -r .token)
  if [ -z "$GITLAB_USER" ] || [ "$GITLAB_USER" = "null" ] || [ -z "$GITLAB_TOKEN" ] || [ "$GITLAB_TOKEN" = "null" ]; then
    echo "ERROR: git_credentials_id=gitlab_token but gitlab-token secret is empty or malformed."
    exit 1
  fi
fi
...
jenkins_config_s3_dir=$(dirname "${jenkins_config_s3_location}")
aws s3 cp "${jenkins_config_s3_dir}/scriptApproval.xml" /var/jenkins_home/scriptApproval.xml
